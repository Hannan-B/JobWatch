...
company end-to-end. Updated 2026-06-30 after the Uber / PlayStation / Deliveroo /
V&A / National Gallery / British Museum / Soho House / Spotify builds._
...
the real request/response in their browser and pastes it. **Build against the
real captured body, never a guessed shape** (DATA_FORMATS §A-3) — guessing the
endpoint/shape is what wasted rounds before captures existed.

What to capture (DevTools → Network → Fetch/XHR → reload):
- The **request** that returns the jobs: its URL, method, and (for POST) body.
  "Copy as cURL" captures all of this faithfully.
- The **response** body (Response tab → copy). This is the source of truth for
  field names.
- Notes on: where the **city** lives, **pagination** (how to get page 2 + any
  total count), whether roles **repeat** per location, and whether it's a
  **global** board.

If the jobs are in the page HTML (no XHR), capture **one job row's outerHTML** and
the table/list **header** structure instead (see §5, HTML pattern).
...
- Page to the board's own reported TOTAL; never an arbitrary cap. Retry a failed
  page once before stopping. If you must stop short, surface "fetched N of M".
...
One `test_<name>.py` at the project root, mirroring the canonical pattern (§7).
...
```python
"""One-paragraph docstring: what this verifies. Run: python3 test_<name>.py"""
import json
...
Key points:
- Tests are **self-contained** (no pytest dependency) — just `python3 test_x.py`.
