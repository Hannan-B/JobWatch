...
- `status` — one of **7**: `applied` (default) · `screening` · `interview` · `ghosted` · `offer` · `rejected` · `withdrawn`. Grouped To-do / In progress / Complete. The live statuses (`applied`/`screening`/`interview`) form a **one-way ladder**; the four terminal statuses are set directly. Backward/sideways live moves are rejected.
...
- **Auto-ghost (locked):** a row still in a live status whose `last_progress_at` is ≥ 14 days ago auto-flips to `ghosted`. Evaluated **lazily on read** (no background timer); the flip is persisted on that read.
